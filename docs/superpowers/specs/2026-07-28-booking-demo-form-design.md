...
4. The bot shows the complete booking summary and an explicit confirmation button.
...
- The demo has no cancellation action. The only restart action is `Новая запись` after a successful confirmation.
